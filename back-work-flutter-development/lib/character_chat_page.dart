...
class _CharacterChatPageState extends State<CharacterChatPage> {
  ...
  Future<void> _generateResponse(String prompt) async {
    ...
    try {
      ...
      final conversationHistory = _messages.take(_messages.length - 1)
          .map((msg) => '${msg.sender == Sender.user ? "User" : widget.character.name}: ${msg.text}')
          .join('\n');
      
      // Use AIChatService with character-specific system prompt
      final responseStream = await AIChatService.generateResponseWithCustomSystem(
        prompt: prompt,
